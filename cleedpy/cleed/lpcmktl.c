...
mat * pc_mktl(mat *p_tl, struct phs_str *phs_shifts, int l_max, real energy)
...
#ifdef ERROR
     fprintf(STDERR,
  "*** error (pc_mktl): %.1f H is lower than the min. energy for set No. %d\n",
       energy, i_set);
     fprintf(STDERR,"\t => exit\n");
#endif
#ifdef EXIT_ON_ERROR
...
#ifdef WARNING
     fprintf(STDWAR,
  "* warning (pc_mktl): %.2f H is higher than the max. energy for set No. %d\n",
       energy, i_set);
