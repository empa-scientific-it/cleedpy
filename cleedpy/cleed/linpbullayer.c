...
Changes:
  ...
  GH/09.12.13 - set MIN_DIST to 1.9 (1.0 A)
...
int inp_bul_layer(struct cryst_str *par,
                  struct atom_str * atom_list, real *a3)
...
#ifdef CONTROL
 fprintf(STDCTR,"(inp_bul_layer): entering inp_bul_layer MIN_DIST= %.3f\n",
         MIN_DIST*BOHR);
#endif
/************************************************************************
...
#ifdef CONTROL
 fprintf(STDCTR,
 "(inp_bul_layer): n_atoms = %d, a1 = (%.3f,%.3f), a2 = (%.3f,%.3f)\n",
...
#ifdef CONTROL
   fprintf(STDCTR,"(inp_bul_layer): pos: %.4f %.4f %.4f dist: %.4f\n",
...
**********************************************************************/
#ifdef CONTROL
     fprintf(STDCTR,"(inp_bul_layer): new layer, no_of_atoms[%d] = %d\n",
...
#ifdef CONTROL
 fprintf(STDCTR,"(inp_bul_layer): setting up bulk layers\n");
#endif
 ...
 *(vec+3*i_layer + 3) = orig[3] + a3[3]
 ...
 if( R_fabs( *(vec + 3*i_layer + 3) ) < MIN_DIST )
 {
   ...
   {
#ifdef ERROR
     fprintf(STDERR," *** error (inp_bul_layer): bulk atoms are too close ");
   ...
   {
#ifdef CONTROL
     fprintf(STDCTR,"(inp_bul_layer): inter layer distance is too short\n");
#endif
/************************************************************************
...
*************************************************************************/
     ...
     atom_list = ( struct atom_str *) realloc(
                 atom_list, (n_atoms+j+2) * sizeof(struct atom_str) );
...
*************************************************************************/
 ...
 for(i=0; i< i_layer; i++)
 {
 fprintf(STDCTR,"(inp_bul_layer): vec_org: %.4f %.4f %.4f layer%d\n",
                 *(vec+3*i+1) *BOHR,
                 *(vec+3*i+2) *BOHR,
                 *(vec+3*i+3) *BOHR,i);
 }
 ...
 for(i=0; i< i_layer; i++)
 {
 fprintf(STDCTR,"(inp_bul_layer): vec_mod: %.4f %.4f %.4f \n",
                 *(vec+3*i+1) *BOHR,
                 *(vec+3*i+2) *BOHR,
                 *(vec+3*i+3) *BOHR);
 }
 fprintf(STDCTR,"(inp_bul_layer):i_layer = %d\n", i_layer);
#endif
...
/* Allocate */
 ...
 par->layers =
       (struct layer_str *) malloc( (i_layer+1) * sizeof(struct layer_str) );
...
#ifdef ERROR
     fprintf(STDERR,
"*** error (inp_bul_layer): the numbers of atoms in layer %d do not match\n", j);
#endif
#ifdef EXIT_ON_ERROR
       exit(1);
#else
...
#ifdef CONTROL
 fprintf(STDCTR,
  "(inp_bul_layer): leaving inp_layer, return value = %d\n", i_layer);
#endif

 return(i_layer);
} /* end of function inp_bul_layer */
/*======================================================================*/
