...
/*********************************************************************
  ...
  The other values of the structure var_par are preset as follows:
    ...
    real vi_exp;  ->  (set in inp_rdpar)
    real theta;   ->  (set in inp_rdpar)
    real phi;     ->  (set in inp_rdpar)
    real k_in[3]; ->  0., 0., 0., 0.
    real epsilon; ->  (set in inp_rdpar)
    int  l_max;   ->  (set in inp_rdpar)
    mat  p_tl;    ->  NULL
...
*********************************************************************/
{
struct var_str *var_par;
struct eng_str *eng_par;
...
#ifdef ERROR
   fprintf(STDERR,
  " *** error (inp_rdpar): could not open file \"%s\"\n",in_file);
#endif
#ifdef EXIT_ON_ERROR
   exit(1);
#else
...
#ifdef WARNING
       fprintf(STDWAR,
  "* warning (inp_rdpar): could not interpret line: \n\t%s\t(in file \"%s\")\n",
   linebuffer, in_file);
#endif
       break;
     }
