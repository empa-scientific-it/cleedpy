...
#ifdef ERROR
   fprintf(STDERR,
  "*** error (inp_rdbul): could not open file \"%s\"\n",filename);
#endif
#ifdef EXIT_ON_ERROR
  exit(1);
#else
...
#ifdef ERROR
             fprintf(STDERR,
               "*** error (inp_rdbul): need at least x/y coordinates of a1\n");
#endif
#ifdef EXIT_ON_ERROR
             exit(1);
#else
...
#ifdef ERROR
             fprintf(STDERR,
               "*** error (inp_rdbul): need at least x/y coordinates of a2\n");
#endif
#ifdef EXIT_ON_ERROR
             exit(1);
#else
...
#ifdef ERROR
             fprintf(STDERR,
               "*** error (inp_rdbul): need x/y coordinates of b1\n");
#endif
#ifdef EXIT_ON_ERROR
             exit(1);
#else
...
#ifdef ERROR
             fprintf(STDERR,
               "*** error (inp_rdbul): need x/y coordinates of b2\n");
#endif
#ifdef EXIT_ON_ERROR
             exit(1);
#else
...
#ifdef WARNING
           fprintf(STDWAR,
"* warning (inp_rdbul): could not interpret line \n\t%s\t(in file \"%s\")\n",
           linebuffer, filename);
#endif
         break;
       }
...
/*
             faux = inp_debtemp(vaux[1] , vaux[2] , bulk_par->temp );
...
#ifdef WARNING
             fprintf(STDWAR,
                 "* warning (inp_rdbul): Could not interpret input: ");
...
#ifdef ERROR
   fprintf(STDERR,
   "*** error (inp_rdbul): could not find any bulk atoms (i_atoms = %d)\n",
   i_atoms);
#endif
#ifdef EXIT_ON_ERROR
   exit(1);
...
#ifdef ERROR
  fprintf(STDERR,
          "*** error (inp_rdbul): superstructure is not commensurate \n");
#endif
#ifdef EXIT_ON_ERROR
  exit(1);
#else
...
#ifdef WARNING
      fprintf(STDWAR,
      "* warning (inp_rdbul): Some coordinates of bulk atoms exceede the\n");
      fprintf(STDWAR,
