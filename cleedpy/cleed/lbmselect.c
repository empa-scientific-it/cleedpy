...
*************************************************************************/
 ...
 for(iaux = 0; (beams_in + iaux)->k_par != F_END_OF_LIST; iaux ++) ;
 iaux ++;

 if (*p_beams_out == NULL)
   *p_beams_out = (struct beam_str *)
                 calloc(iaux, sizeof(struct beam_str));
 else
   *p_beams_out = (struct beam_str *)
                 realloc(*p_beams_out, iaux*sizeof(struct beam_str));

 if(*p_beams_out == NULL)
 {
#ifdef ERROR
   fprintf(STDERR," *** error (bm_select): allocation error.\n");
#endif
   exit(1);
 }
 else
