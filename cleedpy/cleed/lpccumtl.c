...
#ifdef WARNING
   fprintf(STDWAR,
   "* warning (pc_cumtl): input phase shifts are only used up to l_max = %d\n",
   l_max_0);
#endif
 }
...
#ifdef WARNING
   fprintf(STDWAR,
   "* warning (pc_cumtl): input phase shifts exist only up to l_max_0 = %d, for higher l (up to l_max = %d) they are set to zero.\n",
   l_max_0, l_max_t);
#endif
   ;
 }
...
#ifdef WARNING
 fprintf(STDWAR,"(pc_cumtl): All displacements are zero: return Tmat(T=0)\n");
#endif
...
#ifdef CONTROL
   fprintf(STDCTR,"(pc_cumtl): calculate Mx, etc. for l_max = %d\n", l_max_t);
#endif
   pc_mk_ms( &Mx, &My, &Mz, &MxMx, &MyMy, &MzMz, l_max_t);
 }

#ifdef CONTROL_X
 fprintf(STDCTR,"(pc_cumtl): Mx: \n");
 matshow(Mx);
 fprintf(STDCTR,"(pc_cumtl): My: \n");
 matshow(My);
 fprintf(STDCTR,"(pc_cumtl): Mz: \n");
 matshow(Mz);
#endif
...
/* Tmat(T=0) is start value of T_acc for recurrence (i_iter = 0) */
  ...
  */
      ...
      if (i_iter < 4)
      {
      fprintf(STDCTR, "(pc_cumtl): MxMxTn(%d):\n", i_iter-1);
      matshow(MxMxTn);
      fprintf(STDCTR, "(pc_cumtl): MxTnMx(%d):\n", i_iter-1);
      matshow(MxTnMx);
      fprintf(STDCTR, "(pc_cumtl): TnMxMx(%d):\n", i_iter-1);
      matshow(TnMxMx);
      }
#endif
      ...
      if (i_iter < 4)
      {
      fprintf(STDCTR, "(pc_cumtl): MzMzTn(%d):\n", i_iter-1);
      matshow(MzMzTn);
      fprintf(STDCTR, "(pc_cumtl): MzTnMz(%d):\n", i_iter-1);
      matshow(MzTnMz);
      fprintf(STDCTR, "(pc_cumtl): TnMzMz(%d):\n", i_iter-1);
      matshow(TnMzMz);
      }
#endif
...
#ifdef ERROR
   fprintf(STDERR,
   "*** error (pc_cumtl): No convergence after %d iterations\n", i_iter);
#endif
#ifdef EXIT_ON_ERROR
   exit(1);
#else
...
#ifdef CONTROL
 fprintf(STDCTR,"(pc_cumtl): End of function \n");
#endif

 return (Tmat);

} /* end of function pc_cumtl */
