...
int inp_phase( char * phaseinp, real * dr, struct phs_str **p_phs_shifts )
...
#ifdef ERROR
     fprintf(STDERR,
     " *** error (inp_phase) environment variable CLEED_PHASE not defined\n");
#endif
     exit(1);
   }
...
#ifdef ERROR
   fprintf(STDERR,
  " *** error (inp_phase): could not open file \"%s\"\n",filename);
#endif
   exit(1);
 }
...
#ifdef ERROR
   fprintf(STDERR,
   " *** error (inp_phase): unexpected EOF found while reading file \"%s\"\n",
   filename);
#endif
   exit(1);
 }
...
#ifdef ERROR
   fprintf(STDERR,
   " *** error (inp_phase): improper input line in file \"%s\":\n%s",
   filename, linebuffer);
#endif
   exit(1);
 }
...
*/
 if( !strncmp(eng_type,"eV",2) || !strncmp(eng_type,"EV",2) )
 {
   eng_scale = 1./HART;
#ifdef CONTROL
   fprintf(STDCTR,"(inp_phase): Energy input in eV\n");
#endif
 }
