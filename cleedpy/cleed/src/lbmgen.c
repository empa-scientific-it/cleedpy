...
struct beam_str *beams, beam_aux;
struct beam_str *bm_off;
...
**********************************************************************/
 ...
 if (*p_beams == NULL)
  *p_beams = (struct beam_str *)calloc(iaux, sizeof(struct beam_str));
 else
  *p_beams = (struct beam_str *)realloc(*p_beams, iaux*sizeof(struct beam_str));

 if(*p_beams == NULL)
 {
#ifdef ERROR
   fprintf(STDERR," *** error (bm_gen): allocation error.\n");
#endif
   exit(1);
 }
 else
...
#ifdef CONTROL_X
 fprintf(STDCTR,"(bm_gen): eng_max  = %.2f, vr = %.2e\n",
                 eng_max * HART, v_par->vr * HART);
 fprintf(STDCTR,"(bm_gen): dmin  = %.2f, epsilon = %.2e\n",
                 c_par->dmin * BOHR, v_par->epsilon);
 fprintf(STDCTR,"(bm_gen): k_max = %.2f, max. No of beams = %2d\n",
                 k_max, iaux);
#endif
...
#ifdef CONTROL_X
 fprintf(STDCTR,"(bm_gen): a1 = (%.2f, %.2f)",   g1_x, g1_y);
...
**********************************************************************/
 ...
 if( i_set != n_set)
 {
   fprintf(STDWAR,"* warning (bm_gen): wrong number of beam sets found.\n");
...
#ifdef CONTROL_X
 fprintf(STDCTR,"(bm_gen): n1_max = %2d, n2_max = %2d\n", n1_max, n2_max);
#endif
 ...
 fprintf(STDCTR,"(bm_gen): SORTING %2d beams in set %d:\n",
   ...
   for(n1 = offset; n1 < i_beams; n1 ++)
   {
     for(n2 = n1+1; n2 < i_beams; n2 ++)
     {
       ...
       {
         memcpy( & beam_aux, beams + n2, sizeof(struct beam_str) );
         memcpy( beams + n2, beams + n1, sizeof(struct beam_str) );
         memcpy( beams + n1, & beam_aux, sizeof(struct beam_str) );
       }
     } /* n2 */
   }  /* n1 */
