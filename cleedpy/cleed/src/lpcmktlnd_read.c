...
GH/04.05.00
  ...
  pc_mktl_nd(mat *p_tl, struct phs_str *phs_shifts, int l_max, real energy)
...
mat * pc_mktl_nd(mat *p_tl, struct phs_str *phs_shifts, int l_max, real energy)
...
#ifdef CONTROL
 fprintf(STDCTR,"(pc_mktl_nd): energy = %.2f H, n_set = %d, l_max = %d\n",
...
#ifdef CONTROL_X
   fprintf(STDCTR,
           "(pc_mktl_nd):i_set = %d, lmax(set) = %d, neng = %d, t_type = %d\n",
...
*********************************************************/
   ...
   {
#ifdef CONTROL
     fprintf(STDCTR,"(pc_mktl_nd):  d %d: (%s)\n", i_set, ptr->input_file);
#endif
...
#ifdef ERROR
       fprintf(STDERR,
"*** error (pc_mktl_nd): %.1f H is lower than the min. energy for set No. %d\n",
       energy, i_set);
       fprintf(STDERR,"\t => exit\n");
#endif
#ifdef EXIT_ON_ERROR
...
#ifdef WARNING
       fprintf(STDWAR,
"* warning (pc_mktl_nd): %.2f H is higher than max. energy for set No. %d\n",
         energy, i_set);
...
#ifdef CONTROL_X
       fprintf(STDCTR, "(pc_mktl_nd): after pc_temtl, dr[0] = %.3f A^2:\n",
               ptr->dr[0]*BOHR*BOHR);
       matshow(p_tl[i_set]);
#endif
...
#ifdef CONTROL
     fprintf(STDCTR,"(pc_mktl_nd): nd %d: (%s)\n", i_set, ptr->input_file);
#endif
...
"=========================================================================\n");
     fprintf(STDCTR,
             "(pc_mktl_nd): execute external program:\n%s\n", linebuffer);
     fprintf(STDCTR,
"=========================================================================\n");
#endif
...
#ifdef ERROR
       fprintf(STDERR,
       " *** error (pc_mktl_nd): system call %s failed\n", linebuffer);
#endif
       exit(1);
     }

     p_tl[i_set] = inp_mat_lm(p_tl[i_set], l_max, ND_T_MAT_MAT);

#ifdef CONTROL
     fprintf(STDCTR,
             "(pc_mktl_nd): non-diag.Tlm_in for set %d read from file %s:\n",
             i_set, linebuffer);
     matshow(p_tl[i_set]);
#endif
...
#ifdef ERROR
     fprintf(STDERR,
      "*** error (pc_mktl_nd): t_type has no valid value for set No. %d\n",
      i_set);
     fprintf(STDERR,"\t => exit\n");
#endif
#ifdef EXIT_ON_ERROR
