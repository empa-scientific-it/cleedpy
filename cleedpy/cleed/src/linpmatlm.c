...
/********************************************************************/
   ...
   mat inp_mat_lm(mat Mat, int l_max, const char *filename)
...
********************************************************************/
 ...
 {
#ifdef ERROR
   fprintf(STDERR,"\n*** error (inp_mat_lm): could not open file \"%s\"\n",
   filename);
#endif
   exit(1);
 }
...
********************************************************************/
 ...
 {
#ifdef ERROR
   fprintf(STDERR, "\n*** error (inp_mat_lm): unexpected EOF found while reading file \"%s\"\n", filename);
#endif
   exit(1);
 }
 ...
 if (l_max_in > l_max)
   fprintf(STDWAR,
   " * warning (inp_mat_lm): dataset of input file is greater than specified matrix (l_max: input %d, matrix %d)\n",
   l_max_in, l_max);
#endif
...
#ifdef CONTROL
 fprintf(STDCTR, " (inp_mat_lm): l_max_in: %d, eng %f)\n", l_max_in, eng);
#endif
