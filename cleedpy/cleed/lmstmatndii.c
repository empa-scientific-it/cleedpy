...
mat ms_tmat_nd_ii ( mat Tii, mat Llm, mat Tlm_in, int l_max /* arguments */ )
...
#ifdef WARNING
       fprintf(STDWAR,
       "* warning (ms_tmat_nd_ii): increase dimension of Tlm from %d to %d\n",
...
#ifdef WARNING
       fprintf(STDWAR,
       "* warning (ms_tmat_nd_ii): reduce dimension of Tlm from %d to %d\n",
       Tlm_in->rows, Tlm->rows);
#endif
...
#ifdef ERROR
     fprintf(STDERR,
     "*** error (ms_tmat_nd_ii): Tlm does not have the right matrix type (square, complex)\n");
#endif
 }
 else
#ifdef ERROR
   fprintf(STDERR,
     "*** error (ms_tmat_nd_ii): Tlm does not point to a matrix\n");
#endif
...
#ifdef WARNING
   fprintf(STDWAR,
     "* warning (ms_tmat_nd_ii): C.G. coefficients had to be recalculated\n");
#endif
   ;
 }
...
#ifdef CONTROL
 fprintf(STDCTR, "\n(ms_tmat_nd_ii): Tii = Tlm * (1 - Gii * Tlm)^-1:\n");
#endif

 return(Tii);
} /* end of function ms_tmat_nd_ii */
